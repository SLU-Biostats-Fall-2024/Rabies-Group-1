...
```{r}
new_rabies_2 <- mutate(new_rabies, Percent_pos = (Positive.Samples/Number.of.Samples)*100)
```
...
```{r}
SLC <- new_rabies_2 |> filter(County.Name == "St. Lawrence")
head(SLC)
```
...
```{r}
SLC_animals <- SLC |> group_by(Animal.Classification,Received.Year) |> summarise(
  totSub = sum(Number.of.Samples), 
  totPos = sum(Positive.Samples), 
  PPos = (totPos/totSub)*100)
head(SLC_animals, 10)
```
...
```{r}
SLC_recent <- SLC |> filter(Received.Year >= "2020")
head(SLC_recent)
```
