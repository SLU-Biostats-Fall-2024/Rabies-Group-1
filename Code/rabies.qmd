...
Task 1: Load in first 7 columns of the data set 
```{r}
rabies <- read.csv(here("Data","Rabies_Laboratory_Submissions.csv"))
new_rabies <- select(rabies, 1:7)
```

Task 2: Create new column called "Percent_pos"
```{r}
new_rabies$Percent_pos <- (rabies$Positive.Samples/rabies$Number.of.Samples)*100
```

Task 3: What year was the highest total number of submissions received
```{r}
Submissions_by_year <- new_rabies |>
  group_by(Received.Year) |>
    summarize(
      Total_Submissions = sum(Number.of.Samples)
    )
arrange(Submissions_by_year, desc(Total_Submissions))
```

Task 4: What year and country were the highest number of positive samples received 
```{r}
Positive_samples_by_county_and_year <- new_rabies |>
  group_by(County.Name, Received.Year) |>
    summarize(
      Total_Positive_Samples = sum(Positive.Samples)
    )
arrange(Positive_samples_by_county_and_year, desc(Total_Positive_Samples))
```
